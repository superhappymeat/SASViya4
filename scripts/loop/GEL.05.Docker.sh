...
case "$1" in
    ...
    'start')
        ...
        # install a very specific version of containerd
        sudo -u cloud-user bash -c "ansible  sasnode*  -b -m shell  -a 'yum   upgrade -y containerd.io-1.4.3-3.1.el7.x86_64' "
        sudo -u cloud-user bash -c "ansible  sasnode*  -b -m shell  -a 'yum downgrade -y containerd.io-1.4.3-3.1.el7.x86_64' "

...
case "$1" in
    ...
    'start')
        ...
        if  [ "$race_alias" == "sasnode01" ] ;     then
            ...
            sudo -u cloud-user bash -c "cd ~ ; git clone --branch LTS_2020.1 $ADMIN_GIT_CLONE_URL || (cd ~/$ADMIN_GIT_REPO_NAME ; git fetch --all ; git reset --hard origin/$BRANCH)"
            ...
            OBSERVABILITY_GIT_REPO_NAME=PSGEL281-sas-viya-4-observability
            OBSERVABILITY_GIT_CLONE_URL=https://gelgitlab.race.sas.com/GEL/workshops/PSGEL281-sas-viya-4-observability.git
            sudo -u cloud-user bash -c "cd ~ ; git clone --branch main $OBSERVABILITY_GIT_CLONE_URL || (cd ~/$OBSERVABILITY_GIT_REPO_NAME ; git fetch --all ; git reset --hard origin/$BRANCH)"

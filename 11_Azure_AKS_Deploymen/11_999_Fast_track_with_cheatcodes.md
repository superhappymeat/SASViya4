...
* To build the AKS cluster, perform the pre-requisites and deploy Viya 4 using the "manual" way
    ...
    ```sh
    bash -x ~/PSGEL255-deploying-viya-4.0.1*/11_Azure_AKS_*/Standard/11_011_Creating_an_AKS_Cluster.sh 2>&1 \
    | tee -a ~/11_011_Creating_an_AKS_Cluster.log
    bash -x ~/PSGEL255-deploying-viya-4.0.1*/11_Azure_AKS_*/Standard/11_041_Performing_Prereqs_in_AKS.sh 2>&1 \
    | tee -a ~/11_041_Performing_Prereqs_in_AKS.log
    bash -x ~/PSGEL255-deploying-viya-4.0.1*/11_Azure_AKS_*/Standard/Manual/11_042_Deploying_Viya_4_on_AKS.sh 2>&1 \
    | tee -a ~/11_042_Deploying_Viya_4_on_AKS.log
    ```

    Important: it is recommended to send the output into a log file, so, in case of failure, you can investiguate more easily.

  * Optionaly you can run this command to also deploy the monitoring and logging tools in the AKS cluster

    ```sh
    bash -x ~/PSGEL255-deploying-viya-4.0.1*/11_Azure_AKS_*/Standard/Manual/11_061_Install_monitoring_and_logging.sh 2>&1 \
    | tee -a ~/11_061_Install_monitoring_and_logging.log
    ```
...
* To build the AKS cluster, perform the pre-requisites and deploy Viya 4 using the viya4-iac-azure and viya4-deployment docker containers (as described in the "Fully Automated" Hands-on) :
    ...
    ```sh
    bash -x ~/PSGEL255-deploying-viya-4.0.1*/11_Azure_AKS_*/Fully_Automated/11_500_Full_Automation_of_AKS_Deployment.sh 2>&1 \
...
* [11 Azure AKS Deployment / 11 999 Fast track with cheatcodes](/11_Azure_AKS_Deployment/11_999_Fast_track_with_cheatcodes.md)**<-- you are here**
